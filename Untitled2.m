img=imread('lena.jpg');
img1= imnoise(img,'salt & pepper',0.80);
%%
...
imshow(Y);
title('NASFM滤波结果');
imwrite(Y,'NASFM滤波结果.tiff')

subplot(235);
imshow(M);
title('NASFM滤波结果，但是去掉加权求和，仅保留选择性中值滤波');
imwrite(M,'NASFM滤波结果，但是去掉加权求和，仅保留选择性中值滤波.tiff')
...
for i=1:m
    for j=1:n
        for ps=padsize:3
            ...
            if ps==3
            window3= padimg{1}(i:i+1, j:j+1) ;  %取4个pix
            output(i,j)=median(window3(:)); 
            end
        end  
    end                 
end
